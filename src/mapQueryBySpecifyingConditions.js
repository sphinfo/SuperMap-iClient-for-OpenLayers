...
import { TileSuperMapRest, GetFeaturesBySQLParameters, FeatureService } from '@supermap/iclient-ol';
...
function query(){
    ...
    //조회대상
    var param = new GetFeaturesBySQLParameters({
        queryParameter: {
            name: "Countries@World", //Dataset Name or Layer Name
            attributeFilter: "SMID = 234" //필터 조건, SQL문의 WHERE절과 동일
        },
        datasetNames: ["World:Countries"] //[Datasource Name:Dataset Name]
    });

    new FeatureService(dataUrl).getFeaturesBySQL(param, function (serviceResult) {
        //조회결과
